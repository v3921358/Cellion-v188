...
public class MapleFootholdTree {
    ...
    private final List<SwimBoundary> swimfootholds = new ArrayList();
    ...
    private Rectangle fullMapArea;
    private EscortMobFootHold esfh = null;
    private List<Map_BuffZone> buffZone = null;

    private final List<Map_MaplePlatform> movingPlatforms = new ArrayList(); // Moving platform in the game...
    private final List<Pair<String, Integer>> movingPlatformflags = new ArrayList();
    ...
    /**
     * Gets the map area rectangle of the map
     *
     * @return
     */
    public Rectangle getFullMapArea() {
        return fullMapArea;
    }

    /**
     * Updates the full map area recetangle
     */
    public void updateFullMapArea() {
        fullMapArea = new Rectangle(p1.x, p1.y, p2.x - p1.x, p2.y - p1.y);
    }

    public final void addBuffZone(final Map_BuffZone zone) {
        if (this.buffZone == null) {
            this.buffZone = new ArrayList();
        }
        this.buffZone.add(zone);
    }

    public final List<Map_BuffZone> getBuffZone() {
        return buffZone;
    }

    // Escort
    public final void setEscortMobFootHold(final EscortMobFootHold esfh) {
        this.esfh = esfh;
    }

    public EscortMobFootHold getEscortFH() {
        return esfh;
    }

    public final boolean isWithinSwimBoundary(final Point pos) {
        for (SwimBoundary s : swimfootholds) {
            if (s.isWithinSwimBoundary(pos)) {
                return true;
            }
        }
        return false;
    ...
    public void insertSwimAreaBoundary(final int x1, final int x2, final int y1, final int y2) {
        swimfootholds.add(new SwimBoundary(x1, x2, y1, y2));
    }

    public final void addMovingPlatform(Map_MaplePlatform mp) {
        this.movingPlatforms.add(mp);
    }

    public final List<Map_MaplePlatform> getMovingPlatforms() {
        return movingPlatforms;
    }

    public List<Pair<String, Integer>> getMovingPlatformFlags() {
        return Collections.unmodifiableList(movingPlatformflags);
    }

    public void addMovingPlatformflags(Pair<String, Integer> data) {
        movingPlatformflags.add(data);
    }
