...
public class ReactorActionManager extends AbstractPlayerInteraction {
    ...
    public void dropItems(boolean meso, int mesoChance, int minMeso, int maxMeso, int minItems) {
        ...
        for (final ReactorDropEntry d : items) {
            if (d.itemId == 0) {
                ...
                reactor.getMap().spawnItemDrop(reactor, getPlayer(), drop, dropPos, false, false, false);
            }
            dropPos.x += 25;
        }
    }
    ...
    public void dropSingleItem(int itemId) {
        ...
        drop.setGMLog("Dropped from reactor " + reactor.getReactorId() + " on map " + getPlayer().getMapId());
        reactor.getMap().spawnItemDrop(reactor, getPlayer(), drop, reactor.getPosition(), false, false, false);
    }
    ...
    public void cancelHarvest(boolean succ) {
        ...
        getPlayer().getMap().broadcastPacket(getPlayer(), CField.showHarvesting(getPlayer().getId(), 0), false);
        getPlayer().getMap().broadcastPacket(CField.harvestResult(getPlayer().getId(), succ));
    }
