...
public class SkillFactory {
    ...
    public static void load() {
        final MapleData delayData = WzDataStorage.getCharacterWZ().getData("00002000.img");

        final MapleDataProvider datasource = WzDataStorage.getSkillWZ();
        ...
        for (MapleDataFileEntry topDir : root.getFiles()) {
            try {
                String jobXmls = topDir.getName().split(".img")[0];
                jobId = Integer.parseInt(jobXmls);
            } catch (NumberFormatException e) { //We only want to parse job skills in the first loop
                jobId = -1;
            }
            if (jobId >= 0) {
    ...
    public static String getSkillName(final int id) {
        Tuple<String, String, String> skillName = MapleStringInformationProvider.getSkillStringCache().get(id);
        if (skillName != null) {
            return skillName.get_2();
        }
        return "";
    }
