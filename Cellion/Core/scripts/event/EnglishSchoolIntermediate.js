...
function setup() {
    ...
    var eim = em.newInstance("english_i");
    
    eim.setProperty("allow_warpback", "false");
    eim.setProperty("Pass", "false");
    eim.setPropertyEx("GivePass", 3);

    eim.startEventTimer(300000);

    var map = eim.getMapFactory().getMap(702090201);
    map.resetMap();
...
function beginQuest(eim) { // Custom function
    eim.broadcastMapEffect(eim.getMapFactory().getMap(702090201), 1)
}
