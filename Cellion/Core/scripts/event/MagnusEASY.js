...
function setup(eim, leaderid) {
    ...
    var map = eim.setInstanceMap(401060000);
    map.resetFully();
    map.killAllMonsters(false);
    var mob0 = em.getMonster(8880002);
    var modified = em.newMonsterStats();
    modified.setOMp(mob0.getMobMaxMp());
    modified.setOHp(mob0.getMobMaxHp());
    mob0.setOverrideStats(modified);
    eim.registerMonster(mob0);
    map.spawnMonsterOnGroundBelow(mob0, new java.awt.Point(1498, -1347));

    em.setProperty("state", "1");

    eim.startEventTimer(5400000); // 1 hr
    return eim;
}

function playerEntry(eim, player) {
    var map = eim.getMapFactory().getMap(401060000);
...
function end(eim) {
    if (eim.disposeIfPlayerBelow(100, 401060100)) {
        em.setProperty("state", "0");
        em.setProperty("leader", "true");
    }
}
