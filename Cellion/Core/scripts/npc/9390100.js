...
function start() {
	if (cm.getPlayer().getMapId() == 262030300) {
		...
		if (cm.getPlayer().getLevel() < 170) {
			cm.sendOk("There is a level requirement of 170 to attempt Dark Hilla.");
			cm.dispose();
			return;
		}
	...
	status = 0;
		...
		cm.sendOk("You have already went to Hilla in the past 3 hours. Time left: " + cm.getReadableMillis(cm.getCurrentTime(), time + (3 * 3600000)));
		cm.dispose();
		return;
	    }
	...
	// TODO viewing!
		...
		return;
			...
			if (eim == null) {
				...
				if (squd != null) {
	    if (time + (3 * 3600000) >= cm.getCurrentTime() && !cm.getPlayer().isGM()) {
		cm.sendOk("You have already went to Dark Hilla in the past 3 hours. Time left: " + cm.getReadableMillis(cm.getCurrentTime(), time + (3 * 3600000)));
		cm.dispose();
		return;
	    }
					cm.sendYesNo("The squad's battle against the boss has already begun.\r\n" + squd.getNextPlayer());
...
function action(mode, type, selection) {
	...
	case 1:
	    ...
	    break;
	case 2:
		if (!cm.reAdd("DarkDarkHillaBattle", "Hilla")) {
