...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayer().getVMatrixRequirement() && cm.getPlayer().nMobKillsV < 1) {
			...
			text = "I see you still haven't completed my tasks, don't waste my time.\r\n\r\n";
			
			if (cm.getPlayer().nMobKillsV >= 1000) {
				text += "\t#rKill Lv. 200+ Monsters#k (#dComplete#k)\r\n";
			} else {
				text += "\t#rKill Lv. 200+ Monsters#k (#b" + cm.getPlayer().nMobKillsV + "#k / " + "#b1000#k)\r\n";
			}
			if (cm.getPlayer().nMagnusKillsV > 0) {
				text += "\t#rKill Magnus#k (#dComplete#k)\r\n";
			} else {
				text += "\t#rKill Magnus#k (#b" + cm.getPlayer().nMagnusKillsV + "#k / " + "#b1#k)\r\n";
			}
			if (cm.getPlayer().nVellumKillsV > 0) {
				text += "\t#rKill Normal Vellum#k (#dComplete#k)\r\n";
			} else {
				text += "\t#rKill Normal Vellum#k (#b" + cm.getPlayer().nVellumKillsV + "#k / " + "#b1#k)\r\n";
			}
			if (cm.getPlayer().nCrimsonQueenKillsV > 0) {
				text += "\t#rKill Normal Crimson Queen#k (#dComplete#k)\r\n";
			} else {
				text += "\t#rKill Normal Crimson Queen#k (#b" + cm.getPlayer().nCrimsonQueenKillsV + "#k / " + "#b1#k)\r\n";
			}
			if (cm.getPlayer().nVonBonKillsV > 0) {
				text += "\t#rKill Normal Von Bon#k (#dComplete#k)\r\n";
			} else {
				text += "\t#rKill Normal Von Bon#k (#b" + cm.getPlayer().nVonBonKillsV + "#k / " + "#b1#k)\r\n";
			}
			if (cm.getPlayer().nPierreKillsV > 0) {
				text += "\t#rKill Normal Pierre#k (#dComplete#k)\r\n";
			} else {
				text += "\t#rKill Normal Pierre#k (#b" + cm.getPlayer().nPierreKillsV + "#k / " + "#b1#k)\r\n";
			}
        ...
        if (readyToAdvance) {
            if (cm.getPlayer().nMobKillsV > 999 && cm.getPlayer().nMagnusKillsV > 0 && cm.getPlayer().nVellumKillsV > 0 && cm.getPlayer().nCrimsonQueenKillsV > 0 && cm.getPlayer().nVonBonKillsV > 0 && cm.getPlayer().nPierreKillsV > 0) {
                ...
                cm.sendOk("You have yet to complete the required tasks to advance.");
                cm.dispose();
            }
        }
    } else if (status == 2) {
		
		text = "In order to unlock the #dV Matrix#k, you'll need to complete these tasks.\r\n\r\n";
