...
function startChat(type, selection)
{
    if (cm.getMapId() == 180000001) {
        ...
        if (chat == 0)
		//cm.sendSimple("Please give me 696969 Cucumbers.");
            cm.sendSimple("Hello, I can bring you to a map where you can drop untradeable items. Do not abuse this NPC or you will be caught and punished\r\n#L0#I want to go there#l\r\n#L1#I might come back later#l");
        else if (chat == 1) {
            switch (selection) {
                case 1:
                ...
                if (cm.getPlayer().getClient().getChannelServer().getPlayerStorage().getCharacterByName(cm.getText()) != null) {
                    if (cm.getText() != cm.getPlayer().getName()) {
                        if (cm.getPlayer().getClient().getChannelServer().getPlayerStorage().getCharacterByName(cm.getText()).getMapId() == cm.getMapId()) {
							...
							cm.mapChangeTimer(180000001, 910000000, 120, false);
                        } else {
                            cm.sendOk("You and your buddy must be in the same map.");
                        }
                    } else {
