...
public class PacketHelper {
    ...
    public static void addStartedQuestInfo(OutPacket oPacket, User chr) {
        ...
        for (MapleQuestStatus q : started) {
            oPacket.EncodeInteger(q.getQuest().getId()); // Version 174, this is an integer 
    ...
    public static void addCompletedQuestInfo(OutPacket oPacket, User chr) {
        ...
        for (MapleQuestStatus q : completed) {
            ...
            oPacket.EncodeInteger(KoreanDateUtil.getQuestTimestamp(q.getCompletionTime()));
    ...
    public static void addSkillInfo(OutPacket oPacket, User chr) {
        ...
        if (skills != null) {
            ...
            for (Map.Entry<Skill, SkillEntry> skill : skills.entrySet()) {
                oPacket.EncodeInteger(skill.getKey().getId());
                oPacket.EncodeInteger(skill.getValue().skillevel);
                addExpirationTime(oPacket, skill.getValue().expiration);

                if (skill.getKey().isFourthJob()) {
                    oPacket.EncodeInteger(skill.getValue().masterlevel);
                }
            }
            oPacket.EncodeShort(0);
            Map<Integer, Integer> linkskills = new HashMap<>();
            oPacket.EncodeInteger(linkskills.size());
            for (Map.Entry<Integer, Integer> linkskill : linkskills.entrySet()) {
                oPacket.EncodeInteger(linkskill.getKey());
    ...
    public static void addCoolDownInfo(OutPacket oPacket, User chr) {
        ...
        for (MapleCoolDownValueHolder cooling : cd) {
            oPacket.EncodeInteger(cooling.skillId);
            oPacket.EncodeInteger((int) ((cooling.length + cooling.startTime) - cTime) / 1000);
        }
    }
    ...
    public static void addRingInfo(OutPacket oPacket, User chr) {
        ...
        for (MapleRing ring : mRing) {
            oPacket.EncodeInteger(marriageId);
    ...
    public static void addInventoryInfo(OutPacket oPacket, User chr) {
        ...
        *&dummyBLD.nRemainExp64 = CInPacket::Decode8(iPacket);
        ...
        oPacket.EncodeInteger(0); // if > 0, write that many ints

        oPacket.Encode(0); // 9 ints, 3 buffers (8)
        oPacket.Encode(0); // byte, int, byte, 6 ints, string
        oPacket.Encode(0); // byte, int, byte, 6 ints, string

        oPacket.Encode(chr.getInventory(MapleInventoryType.EQUIP).getSlotLimit());
        oPacket.Encode(chr.getInventory(MapleInventoryType.USE).getSlotLimit());
        oPacket.Encode(chr.getInventory(MapleInventoryType.SETUP).getSlotLimit());
        oPacket.Encode(chr.getInventory(MapleInventoryType.ETC).getSlotLimit());
        oPacket.Encode(chr.getInventory(MapleInventoryType.CASH).getSlotLimit());
        ...
        oPacket.Encode(0); // End of USE Inventory
        ...
        oPacket.Encode(0); // End of Setup Inventory
        ...
        oPacket.Encode(0); // End of ETC Inventory
        ...
        oPacket.Encode(0); // End of CASH Inventory

//        for (int i = 0; i < chr.getExtendedSlots().size(); i++) {
//            oPacket.encodeInteger(i);
//            oPacket.encodeInteger(chr.getExtendedSlot(i));
...
//        }
    ...
    public static void addPotionPotInfo(OutPacket oPacket, User chr) {
        ...
        oPacket.EncodeInteger(chr.getPotionPots().size());
        for (MaplePotionPot p : chr.getPotionPots()) {
            oPacket.EncodeInteger(p.getId());
            oPacket.EncodeInteger(p.getMaxValue());
            oPacket.EncodeInteger(p.getHp());
            oPacket.EncodeInteger(0);
            oPacket.EncodeInteger(p.getMp());
    ...
    public static void addCharStats(OutPacket oPacket, User chr) {
        ...
        oPacket.Encode(chr.getSkinColor());
        oPacket.EncodeInteger(chr.getFace());
        oPacket.EncodeInteger(chr.getHair());
        ...
        oPacket.EncodeInteger(0); // F3 4F 11 00
        oPacket.EncodeInteger(chr.getGachExp());
        oPacket.EncodeInteger(chr.getMapId());
        oPacket.Encode(chr.getInitialSpawnpoint());
        oPacket.EncodeInteger(0); // playtime in seconds?
        ...
        if (GameConstants.isDemonSlayer(chr.getJob()) || GameConstants.isXenon(chr.getJob()) || GameConstants.isDemonAvenger(chr.getJob()) || GameConstants.isBeastTamer(chr.getJob())) {
            oPacket.EncodeInteger(chr.getFaceMarking());
        }

        // Professions
        oPacket.Encode(chr.getFatigue());
        oPacket.EncodeInteger(GameConstants.getTimeAsInt());
        for (MapleTrait.MapleTraitType t : MapleTrait.MapleTraitType.values()) {
            oPacket.EncodeInteger(chr.getTrait(t).getTotalExp());
        }
        for (MapleTrait.MapleTraitType t : MapleTrait.MapleTraitType.values()) {
            oPacket.EncodeShort(0);
        }
        ...
        oPacket.EncodeInteger(chr.getStat().pvpExp);
        oPacket.Encode(chr.getStat().pvpRank);
        oPacket.EncodeInteger(chr.getBattlePoints());
        oPacket.Encode(5); // nPvPModeLevel
        ...
        oPacket.EncodeInteger(0);
        oPacket.EncodeInteger(0);
        oPacket.Encode(chr.isBurning()); //bBurning
        oPacket.EncodeInteger(0);
        oPacket.EncodeInteger(0);
    }
    ...
    public static void addCharLook(OutPacket oPacket, MapleCharacterLook chr, boolean mega, boolean second) {
        oPacket.Encode(second ? chr.getSecondaryGender() : chr.getGender());//Secondary gender is always 1 (zeroBeta/AB) but meh
        oPacket.Encode(chr.getSkinColor());//AB is always white and Zero shares skin color, so no need for secondSkin.
        oPacket.EncodeInteger(second ? secondaryFace(chr) : chr.getFace());
        oPacket.EncodeInteger(chr.getJob());
        oPacket.Encode(mega ? 0 : 1);
        oPacket.EncodeInteger(second ? secondaryHair(chr) : chr.getHair());
        ...
        oPacket.EncodeInteger(!zero && Shield != null ? Shield : 0);
        oPacket.Encode(GameConstants.isMercedes(chr.getJob()) ? 1 : 0); // Mercedes/Elf Ears
        oPacket.Encode(new byte[12]);
        if (GameConstants.isDemonSlayer(chr.getJob()) || GameConstants.isXenon(chr.getJob()) || GameConstants.isDemonAvenger(chr.getJob()) || GameConstants.isBeastTamer(chr.getJob())) {
            oPacket.EncodeInteger(chr.getFaceMarking());
        }
        if (GameConstants.isZero(chr.getJob())) {
            oPacket.Encode(second);
        }
        if (GameConstants.isBeastTamer(chr.getJob())) {
            oPacket.Encode(1);
            oPacket.EncodeInteger(chr.getEars());
            oPacket.Encode(1);
            oPacket.EncodeInteger(chr.getTail());
        }
        oPacket.Encode(0); // nMixedHairColor
        oPacket.Encode(0); // nMixHairPercent
    }
    ...
    public static void addItemPosition(OutPacket oPacket, Item item, boolean trade, boolean bagSlot) {
        ...
        if (bagSlot) {
            oPacket.EncodeInteger(pos % 100 - 1);
    ...
    public static void addItemInfo(final OutPacket oPacket, final Item item, final User chr) {
        ...
        if (item.getPet() != null) {
            ...
            oPacket.EncodeInteger(chr == null ? -1 : chr.getExtendedSlots().indexOf(item.getItemId()));
    ...
    public static void addEquipStats(OutPacket oPacket, Equip equip, List<EquipStat> eqStats, List<EquipSpecialStat> eqSpecialStats) {
        ...
        oPacket.EncodeInteger(head);

        if (head != 0) {
            if (eqStats.contains(EquipStat.SLOTS)) {
                oPacket.Encode(equip.getUpgradeSlots());
            }
            if (eqStats.contains(EquipStat.LEVEL)) {
                oPacket.Encode(equip.getLevel());
            }
            ...
            if (eqStats.contains(EquipStat.INC_SKILL)) {//nLevelUpType
                oPacket.Encode(equip.getIncSkill() > 0 ? 1 : 0);
            }
            if (eqStats.contains(EquipStat.ITEM_LEVEL)) {//nLevel
                oPacket.Encode(Math.max(equip.getBaseLevel(), equip.getEquipLevel())); // Item level
            }
            ...
            if (eqStats.contains(EquipStat.DURABILITY)) {
                oPacket.EncodeInteger(equip.getDurability());
            }
            if (eqStats.contains(EquipStat.VICIOUS_HAMMER)) {//nIUC
                oPacket.EncodeInteger(equip.getViciousHammer());
            }
...
//            oPacket.encode(0); //unknown
    ...
    public static void addEquipBonusStats(OutPacket oPacket, Equip equip, boolean hasUniqueId) {
        ...
        oPacket.Encode(equip.getPotentialTier().getValue());
        oPacket.Encode(equip.getEnhance());
        ...
        oPacket.EncodeInteger(equip.getExpBonus()); // this is nPrevBonusExpRate

        oPacket.EncodeLong(equip.getUniqueId());
        oPacket.EncodeLong(getTime(-2)); //ftExpireDate

        oPacket.EncodeInteger(equip.getCashGrade() / 1000); //nGrade
        for (int i = 0; i < 3; i++) {//hardcoded 3 iterations of int
            oPacket.EncodeInteger(equip.getCashStats()[i] % 1000);
        }
        ...
        if (ItemConstants.IsArcaneSymbol(equip.getItemId())) {
            ...
            oPacket.EncodeInteger(equip.getArcaneExp());
    ...
    public static void serializeMovementList(OutPacket oPacket, MapleMapObject obj, List<LifeMovementFragment> moves, int bPassive) {
        if (obj != null) {
            oPacket.EncodeInteger(obj.gettEncodedGatherDuration());
    ...
    public static void addInteraction(OutPacket oPacket, IMaplePlayerShop shop) {
        oPacket.Encode(shop.getGameType());
        oPacket.EncodeInteger(((AbstractPlayerStore) shop).getObjectId());
        oPacket.EncodeString(shop.getDescription());
        if (shop.getShopType() != 1) {
            oPacket.Encode(shop.getPassword().length() > 0 ? 1 : 0);
        }
        oPacket.Encode(shop.getItemId() % 10);
        oPacket.Encode(shop.getSize());
        oPacket.Encode(shop.getMaxSize());
        if (shop.getShopType() != 1) {
            oPacket.Encode(shop.isOpen() ? 0 : 1);
        }
    }
    ...
    public static void addCharacterInfo(OutPacket oPacket, User chr) {
        ...
        for (int i = 0; i < 3; i++) {
            oPacket.EncodeInteger(-20); //skill cooldown time aPetActiveSkillCoolTime
        }
        oPacket.Encode(0);
        oPacket.Encode(0); // loop if byte > 0, decode an int
        oPacket.EncodeInteger(0); // (if int > 0, write an int and decodebuffer)
        ...
        if ((mask & 1) != 0) {
            ...
            oPacket.Encode(chr.getBuddylist().getCapacity());

            oPacket.Encode(chr.getBlessOfFairyOrigin() != null);//correct
            if (chr.getBlessOfFairyOrigin() != null) {
                oPacket.EncodeString(chr.getBlessOfFairyOrigin());
            }

            oPacket.Encode(chr.getBlessOfEmpressOrigin() != null);//correct
            ...
            oPacket.Encode((ultExplorer != null) && (ultExplorer.getCustomData() != null));//correct
        ...
        if ((mask & 0x100000) != 0) {//correct
            oPacket.EncodeInteger(chr.getMonsterBook().getSet());
        }
        ...
        oPacket.Encode(0); // v178 new
        // if > 0, for loop int, encode int, str per size

        if ((mask & 0x1000) != 0) {//correct
            oPacket.EncodeInteger(0);//if this int > 0, write two ints
            /*
        ...
        oPacket.EncodeInteger(0); //new v134 //correct
        oPacket.Encode(0);//correct
        ...
        if ((mask & 4) != 0) {//correct
            oPacket.Encode(0); // if this byte > 0, decode item slot and write an int
        }

        if ((mask & 8) != 0) {//correct
            if (GameConstants.isAngelicBuster(chr.getJob())) {
                //This might actually ffing be where if(zero) write betalook
                oPacket.EncodeInteger(21173); //face
                oPacket.EncodeInteger(37141); //hair
                oPacket.EncodeInteger(1051291); // dressup suit cant unequip
            } else {
                oPacket.EncodeInteger(0);
                oPacket.EncodeInteger(0);
                oPacket.EncodeInteger(0);
            ...
            oPacket.Encode(0);
            oPacket.EncodeInteger(-1); //nMixBaseHairColor
            oPacket.EncodeInteger(0); //nMixAddHairColor
            oPacket.EncodeInteger(0); //nMixHairBaseProb
        }
        ...
        if ((mask & 0x20) != 0) {//correct
            oPacket.EncodeInteger(0); // if > 0, write int and buffer
        }

        if ((mask & 0x40) != 0) {//correct
            addFarmInfo(oPacket, chr.getClient(), 0);
            oPacket.EncodeInteger(1);//v146 can be 5 tho... its 1 in v169
            oPacket.EncodeInteger(0);
        }

        if ((mask & 0x80) != 0) {//correct
            oPacket.Encode(0); // if this byte > 0, decode an item slot, and two int
            /*
        ...
        if ((mask & 0x20000) != 0) {
            oPacket.EncodeInteger(chr.getId());
            oPacket.EncodeInteger(0); // nLastScore
            oPacket.EncodeInteger(0); // nHighScore
            oPacket.EncodeInteger(0); // nRunnerPoint
            oPacket.EncodeLong(getTime(-2));
            oPacket.EncodeInteger(10); // nTotalLeft
        }
        ...
        oPacket.Encode(0); // new v178
        oPacket.EncodeInteger(0);// EncodeTextEquipInfo
        ...
        if ((mask & 0x200000) != 0) {
            oPacket.EncodeInteger(chr.aVMatrixRecord.size());
        ...
        if ((mask & 0x4000000) != 0) {//correct
            oPacket.Encode(0); // if this byte > 0, decode a byte, three ints and a long
        ...
        if ((mask & 0x10000000) != 0) {//correct
            oPacket.EncodeInteger(0); // if this int > 0, decode 2 shorts
            oPacket.EncodeInteger(0); // if this int > 0, decode a short and an int
        }
    ...
    public static void addAbilityInfo(final OutPacket oPacket, User chr) {
        ...
        for (int i = 0; i < skills.size(); ++i) {
            oPacket.Encode(i + 1); // key
            oPacket.EncodeInteger(skills.get(i).getSkillId()); //d 7000000 id ++, 71 = char cards
            oPacket.Encode(skills.get(i).getSkillLevel()); // level
            oPacket.Encode(skills.get(i).getRank()); //rank, C, B, A, and S
        }
    ...
    public static void addCoreAura(OutPacket oPacket, User chr) {
        ...
        oPacket.EncodeInteger(aura.getExpire()); // timer
        oPacket.EncodeInteger(0);

        oPacket.EncodeInteger(aura.getAtt()); // weapon attack
        oPacket.EncodeInteger(aura.getDex()); // dex
        oPacket.EncodeInteger(aura.getLuk()); // luk
        oPacket.EncodeInteger(aura.getMagic()); // magic att
        oPacket.EncodeInteger(aura.getInt()); // int
        oPacket.EncodeInteger(aura.getStr()); // str

        oPacket.EncodeInteger(0);
        oPacket.EncodeInteger(aura.getTotal());//max
        oPacket.EncodeInteger(0);
        oPacket.EncodeInteger(0);
    ...
    public static void addPetItemInfo(OutPacket oPacket, Item item, Pet pet, boolean active) {
        ...
        oPacket.EncodeInteger((pet.getItem().getItemId() == 5000054) && (pet.getSecondsLeft() > 0) ? pet.getSecondsLeft() : 0);
        oPacket.EncodeShort(0); //_ZtlSecureTear_nAttribute
        oPacket.Encode(active ? 0 : pet.getSummoned() ? pet.getSummonedValue() : 0);
        oPacket.EncodeInteger(0);// nAutoBuffSkill (I think this is the skill that the player can set so the pet autobuffs
        oPacket.EncodeInteger(pet.getColor()); //nPetHue
    ...
    public static void addShopInfo(OutPacket oPacket, MapleShop shop, MapleClient c) {
        ...
        oPacket.EncodeInteger(0);
        oPacket.EncodeInteger(shop.getNpcId()); //m_nSelectNpcItemID
        oPacket.EncodeInteger(0);
        oPacket.EncodeInteger(0); //m_nStarCoin
    ...
    /*public static void addShopItemInfo(OutPacket oPacket, MapleShopItem item, MapleShop shop, MapleItemInformationProvider ii, Item repurchaseItemInfo, MapleCharacter chr) {
        ...
        oPacket.EncodeInteger(item.getReqItem());
        oPacket.EncodeInteger(item.getReqItemQ());
        oPacket.EncodeInteger(item.getPointQuestId());//nPointQuestID
        oPacket.EncodeInteger(item.getPointQuestPrice());//nPointPrice
        oPacket.EncodeInteger(item.getStarCoin());//nStarCoin
        oPacket.EncodeInteger(item.getQuestExId());//nQuestExID
        oPacket.EncodeString("");//item.getQuestExKey());//&item.sQuestExKey
        oPacket.EncodeInteger(item.getQuestExValue());//nQuestExValue
        oPacket.EncodeInteger(1440 * item.getExpiration());
        oPacket.EncodeInteger(0);//nLevelLimited
        oPacket.EncodeShort(item.getMinLevel());
        oPacket.EncodeShort(item.getMaxLevel());//nShowLevMax
        oPacket.EncodeInteger(item.getQuestId());//nQuestID
        oPacket.EncodeLong(getTime(-2));
        oPacket.EncodeLong(getTime(-1));
        oPacket.EncodeInteger(item.getCategory());
        oPacket.Encode(0);//bWorldBlock
        oPacket.EncodeInteger(item.getPotentialGrade());
        oPacket.EncodeInteger(item.getSaleLimit());//nBuyLimit
    ...
    public static void addZeroInfo(OutPacket oPacket, User chr) {
        ...
        if ((mask & 1) != 0) {
            oPacket.Encode(chr.isZeroBetaState());
        }
        if ((mask & 2) != 0) {
            oPacket.EncodeInteger(chr.getStat().getHp()); //hp
        }
        if ((mask & 4) != 0) {
            oPacket.EncodeInteger(chr.getStat().getMp()); //mp
        }
        if ((mask & 8) != 0) {
            oPacket.Encode(chr.getSkinColor());
        }
        if ((mask & 0x10) != 0) {
            oPacket.EncodeInteger(chr.getZeroBetaHair());
        }
        if ((mask & 0x20) != 0) {
            oPacket.EncodeInteger(chr.getZeroBetaHair());
        }
        if ((mask & 0x40) != 0) {
            oPacket.EncodeInteger(chr.getMaxHp()); //maxhp
        }
        if ((mask & 0x80) != 0) {
            oPacket.EncodeInteger(chr.getMaxMp()); //maxmp
        }
        if ((mask & 0x100) != 0) {
            oPacket.EncodeInteger(0); //dbcharZeroLinkCashPart
        }
        if ((mask & 0x200) != 0) {
            oPacket.EncodeInteger(0); //nMixBaseHairColor
            oPacket.EncodeInteger(0); //nMixAddHairColor
            oPacket.EncodeInteger(0); //nMixHairBaseProb
        }
    }
    ...
    public static void addBeastTamerInfo(OutPacket oPacket, User chr) {
        ...
        oPacket.encodeString("bTail=" + beast + ";", false);
        oPacket.encodeString("bEar=" + beast + ";", false);
        oPacket.encodeString("TailID=" + tail + ";", false);
        oPacket.encodeString("EarID=" + ears, false);
    ...
    public static void addFarmInfo(OutPacket oPacket, MapleClient c, int gender) {
        ...
        oPacket.EncodeInteger(c.getFarm().getWaru());
        oPacket.EncodeInteger(c.getFarm().getLevel());
        oPacket.EncodeInteger(c.getFarm().getExp());
        oPacket.EncodeInteger(c.getFarm().getAestheticPoints());
        oPacket.EncodeInteger(0); //gems

        oPacket.Encode(gender);
        oPacket.EncodeInteger(0); //farm theme
        oPacket.EncodeInteger(0); // slot extension
    ...
    public static void addRedLeafInfo(OutPacket oPacket, User chr) {
        ...
        oPacket.EncodeInteger(0);
        for (int i = 0; i < 4; i++) {
            oPacket.EncodeInteger(9410165 + i);//v146 -2
            oPacket.EncodeInteger(chr.getFriendShipPoints()[i]);
        }

    }

    public static void addLuckyLogoutInfo(OutPacket oPacket, boolean enable, CashItem item0, CashItem item1, CashItem item2) {
        oPacket.EncodeInteger(enable ? 1 : 0);
        if (enable) {
    ...
    public static void addPartTimeJob(OutPacket oPacket, PartTimeJob parttime) {
        oPacket.Encode(parttime.getJob());
        if (parttime.getJob() > 0 && parttime.getJob() <= 5) {
            oPacket.EncodeLong(parttime.getTime());
        } else {
            oPacket.EncodeLong(getTime(-2));
        }
        oPacket.EncodeInteger(parttime.getReward());
        oPacket.Encode(parttime.getReward() > 0);
    }
    ...
    public static <E extends TemporaryStat> void writeSingleMask(OutPacket oPacket, E statup) {
        for (int i = GameConstants.CFlagSize; i >= 1; i--) {
            oPacket.EncodeInteger(i == statup.getPosition() ? statup.getValue() : 0);
        }
    }
    ...
    public static <E extends TemporaryStat> void writeMask(OutPacket oPacket, Collection<E> statups) {
        ...
        for (int i = mask.length; i >= 1; i--) {
            oPacket.EncodeInteger(mask[(i - 1)]);
        }
    }
