...
public class MapleInventory implements Iterable<Item> {
    ...
    public void addSlot(byte slot) {
        ...
        if (this.slotLimit > GameConstants.maxInventorySlot) {
            this.slotLimit = GameConstants.maxInventorySlot;
        }
    }
    ...
    public void setSlotLimit(byte slot) {
        if (slot > GameConstants.maxInventorySlot) {
            slot = GameConstants.maxInventorySlot;
        }
        this.slotLimit = slot;
    }
    ...
    /**
     * Finds an array of item from the inventory
     *
     * @param items
     * @return True if any of the items in the array are available
     */
    public boolean findMultipleItemsById(int... items) {
        for (Item item : this.inventory.values()) {
            for (int i : items) {
                if (i == item.getItemId()) {
                    return true;
    ...
    public short addShield(Item item) {
        short slotId = 97;
