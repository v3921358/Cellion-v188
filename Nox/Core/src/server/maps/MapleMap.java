...
public final class MapleMap {
    ...
    private void dropFromMonster(final User chr, final MapleMonster mob, final boolean instanced) {
    ...
    public void removeMonster(MapleMonster monster) {
        ...
        broadcastMessage(MobPacket.killMonster(monster.getObjectId(), 0, GameConstants.isAzwanMap(smr.mapid)));
    ...
    public final void killMonster(MapleMonster monster, User chr, boolean withDrops, boolean second, short animation) {
        killMonster(monster, chr, withDrops, second, animation, 0);
    }

    public final void killMonster(MapleMonster monster, User chr, boolean withDrops, boolean second, short animation, int lastSkill) {
                ...
                for (final User mc : characters) {
                    if (mc.isAlive()) {
                        ...
                        switch (monster.getId()) {
                            ...
                            case 8810122:
                            case 8820001:
                                mc.getClient().write(EffectPacket.showOwnBuffEffect(buffid, UserEffectCodes.BuffItemEffect, mc.getLevel(), 1)); // HT nine spirit
                                broadcastMessage(mc, EffectPacket.showBuffeffect(mc.getId(), buffid, UserEffectCodes.BuffItemEffect, mc.getLevel(), 1), false); // HT nine spirit
                                break;
                        }
                    }
                }
...
//            FileoutputUtil.log(FileoutputUtil.Horntail_Log, MapDebug_Log());
            ...
            World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6, "To you whom have defeated Arkarium in the Darkness, you are the heroes of time!"));
            charactersLock.readLock().lock();
            try {
                for (User c : characters) {
                    // c.finishAchievement(39);
            ...
            World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6, "The Brave heroes have finally killed Chaos Magnus, Cheers Maplers.."));
            charactersLock.readLock().lock();
            try {
                for (User c : characters) {
                }
            ...
            World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6, "The brave warriors of Ereve destroyed Ranmaru and now peace sorrounds the vail."));
            charactersLock.readLock().lock();
            try {
                for (User c : characters) {
                }
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
            ...
            switch ((chr.getMapId() % 1000) / 100) {
                ...
                case 3:
                case 4:
                    chr.getClient().write(CField.MapEff("monsterPark/clear"));
                    break;
                case 5:
                    if (chr.getMapId() / 1000000 == 952) {
                        chr.getClient().write(CField.MapEff("monsterPark/clearF"));
                    } else {
                        chr.getClient().write(CField.MapEff("monsterPark/clear"));
                    }
                    break;
    ...
    public void killMonster(final int monsId) {
        for (MapleMapObject monstermo : this.getAllMapObjects(MapleMapObjectType.MONSTER)) {
            ...
            if (monster.getId() == monsId) {
                ...
                removeMapObject(monster);
                if (GameConstants.isAzwanMap(smr.mapid)) {
                    broadcastMessage(MobPacket.killMonster(monster.getObjectId(), 1, true));
                } else {
                    broadcastMessage(MobPacket.killMonster(monster.getObjectId(), 1, false));
                }
                monster.killed();
                break;
            }
    ...
    public void spawnNpcForPlayer(MapleClient c, final int id, final Point pos) {
        final MapleNPC npc = MapleLifeFactory.getNPC(id);
        npc.setPosition(pos);
        npc.setCy(pos.y);
        npc.setRx0(pos.x + 50);
    ...
    public void spawnNpcOnMapLoad(MapleNPC life) {
        spawnAndAddRangedMapObject(life, (MapleClient c1) -> {
    ...
    public final void hideNpc(MapleClient c, final int npcid) {
        ...
        try {
            ...
            while (itr.hasNext()) {
                MapleNPC npc = (MapleNPC) itr.next();
                if (npcid == -1 || npc.getId() == npcid) {
                    c.write(NPCPacket.removeNPCController(npc.getObjectId()));
                    c.write(NPCPacket.removeNPC(npc.getObjectId()));
                }
            }
        } finally {
            mapobjectlocks.get(MapleMapObjectType.NPC).readLock().unlock();
    ...
    public void spawnClockMist(final MapleMist clock) {
        spawnAndAddRangedMapObject(clock, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                c.write(CField.spawnClockMist(clock));
            }
        });
        MapTimer.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                broadcastMessage(CField.removeMist(clock.getObjectId(), false));
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAzwanMap(c.getPlayer().getMapId())) {
                    c.write(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(), oid, true)); // TODO effect
                } else {
                    c.write(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(), oid, false)); // TODO effect
                }
            }
        });
    ...
    public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite, final User chr) {
        //  MapleClient c;
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAzwanMap(c.getPlayer().getMapId())) {
                    c.write(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : monster.getStats().getSummonType(), 0, true));
                } else if (GameConstants.isChangeable(monster.getId())) {
                    c.write(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || true ? spawnType : monster.getStats().getSummonType(), 0, false));
                } else {
                    c.write(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : monster.getStats().getSummonType(), 0, false));
                }
            }
        });
        updateMonsterController(monster);
    ...
    public final int spawnMonsterWithEffect(final MapleMonster monster, final int effect, Point pos) {
        try {
            ...
            spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
                ...
                public final void sendPackets(MapleClient c) {
                    if (GameConstants.isAzwanMap(c.getPlayer().getMapId())) {
                        c.write(MobPacket.spawnMonster(monster, effect, 0, true));
                    } else {
                        c.write(MobPacket.spawnMonster(monster, effect, 0, false));
                    }
                }
            });
            updateMonsterController(monster);
    ...
    public final void spawnFakeMonster(final MapleMonster monster) {
        ...
        spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                if (GameConstants.isAzwanMap(c.getPlayer().getMapId())) {
                    c.write(MobPacket.spawnMonster(monster, -4, 0, true));
                } else {
                    c.write(MobPacket.spawnMonster(monster, -4, 0, false));
                }
            }
        });
        updateMonsterController(monster);
    ...
    public final void spawnReactor(final MapleReactor reactor) {
        ...
        spawnAndAddRangedMapObject(reactor, new DelayedPacketCreation() {
            @Override
            public final void sendPackets(MapleClient c) {
                c.write(CField.spawnReactor(reactor));
            }
        });
    }
    ...
    public final void spawnMechDoor(final MechDoor door) {
        spawnAndAddRangedMapObject(door, new DelayedPacketCreation() {
            @Override
            public final void sendPackets(MapleClient c) {
                c.write(CField.spawnMechDoor(door, true));
                c.write(CWvsContext.enableActions());
            }
        });
    }
        ...
        spawnAndAddRangedMapObject(summon, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                if (summon != null && c.getPlayer() != null && (!summon.isChangedMap() || summon.getOwnerId() == c.getPlayer().getId())) {
                    c.write(SummonPacket.spawnSummon(summon, true));
                }
            }
        });
    }
    ...
    public final void spawnFamiliar(final MonsterFamiliar familiar, final boolean respawn) {
        spawnAndAddRangedMapObject(familiar, (MapleClient c1) -> {
            if (familiar != null && c1.getPlayer() != null) {
                c1.write(CField.spawnFamiliar(familiar, true, respawn));
            }
        });
    }

    public final void spawnExtractor(final MapleExtractor ex) {
        spawnAndAddRangedMapObject(ex, (MapleClient c1) -> {
            ex.sendSpawnData(c1);
        });
    }
    ...
    public final void spawnMesoDrop(int meso, Point position, MapleMapObject dropper, User owner, boolean playerDrop, byte droptype) {
        ...
        if (dropper != null) {
            if (dropper instanceof MapleMonster) {
                mdrop.setProperties(MapleMapItemProperties.IsPickpocketDrop);
            }
        }
        spawnAndAddRangedMapObject(mdrop, (MapleClient c1) -> {
            c1.write(CField.dropItemFromMapObject(mdrop, dropper.getTruePosition(), droppos, (byte) 1));
        });
        if (!smr.everlast) {
     ...
     * @param global_drop
     */
    public final void spawnMobDrop(Item idrop, Point dropPos, MapleMonster mob, User chr, byte droptype, int questid, boolean global_drop) {
        ...
        spawnAndAddRangedMapObject(mdrop, (MapleClient c1) -> {
            if (c1 != null && c1.getPlayer() != null && (questid <= 0 || c1.getPlayer().getQuestStatus(questid) == MapleQuestState.Started) && (idrop.getItemId() / 10000 != 238 || c1.getPlayer().getMonsterBook().getLevelByCard(idrop.getItemId()) >= 2) && mob != null && dropPos != null) {
                c1.write(CField.dropItemFromMapObject(mdrop, mob, mob.getTruePosition(), dropPos, (byte) 1));
            }
        });
//	broadcastMessage(CField.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 0));
    ...
    private void spawnItemDropInternal(MapleMapObject dropper, User owner, boolean showToOwnerOnly, Item item, Point pos,
        ...
        spawnAndAddRangedMapObject(drop, (MapleClient c1) -> {
            if ((showToOwnerOnly && owner != null && owner.getId() == c1.getPlayer().getId())
                    || !showToOwnerOnly) {
                c1.write(CField.dropItemFromMapObject(drop, dropper.getTruePosition(), droppos, (byte) 1));
            }
        });
        if (showToOwnerOnly) {
    ...
    public final void spawnAutoDrop(final int itemid, final Point pos) {
        ...
        spawnAndAddRangedMapObject(mdrop, (MapleClient c1) -> {
            c1.write(CField.dropItemFromMapObject(mdrop, pos, pos, (byte) 1));
        });
     ...
     */
    public final void addPlayer(final User chr, int lastmapid) {
        ...
        chr.setChangeTime();
        if (GameConstants.isTeamMap(smr.mapid) && !chr.inPVP()) {
            chr.setTeam(getAndSwitchTeam() ? 0 : 1);
        }

        if (!chr.isHidden()) {
            broadcastMessage(chr, CField.spawnPlayerMapObject(chr), false);
            if (chr.isIntern() && speedRunStart > 0) {
                endSpeedRun();
                broadcastMessage(CWvsContext.broadcastMsg(5, "The speed run has ended."));
            }
        } else {
            broadcastGMMessage(chr, CField.spawnPlayerMapObject(chr), false);
        }
                ...
                default:
                    if (!smr.onFirstUserEnter.equals("")) {
                        if (getCharactersSize() == 1) {
                            MapScriptManager.executeMapScript(chr.getClient(), smr.onFirstUserEnter, true, lastmapid);
                        }
                    }
                    if (!smr.onUserEnter.equals("")) {
                        MapScriptManager.executeMapScript(chr.getClient(), smr.onUserEnter, false, lastmapid);
                    }
                    break;
            }
        }
        ...
        for (final Pet pet : chr.getPets()) {
            if (pet.getSummoned()) {
                broadcastMessage(chr, PetPacket.OnActivated(chr.id, chr.getPetIndex(pet), true, pet, 0), false);
            }
        }
        ...
        if (chr.getParty() != null) {
            ...
            chr.getClient().write(PartyPacket.updateParty(chr.getClient().getChannel(), chr.getParty(), PartyOperation.SILENT_UPDATE, null));
            chr.updatePartyMemberHP();
            chr.receivePartyMemberHP();
        }
        if (!chr.isInBlockedMap() && chr.getLevel() > 10) {
            for (QuickMove qm : QuickMove.values()) {
                if (qm.getMap() == chr.getMapId() || ServerConstants.QUICKMOVE_EVERYWHERE) {
                    ...
                    chr.getClient().write(CField.getQuickMoveInfo(true, qmn));
                    break;
                }
            }
        } else {
        ...
        if (GameConstants.isPhantom(chr.getJob())) {
            chr.getClient().write(PhantomPacket.updateCardStack(chr.getCardStack()));
        }
        if (mapEffect != null) {
        ...
        if (chr.getEventInstance() != null && chr.getEventInstance().isTimerStarted()) {
            if (chr.inPVP()) {
                chr.getClient().write(CField.getPVPClock(Integer.parseInt(chr.getEventInstance().getProperty("type")), (int) (chr.getEventInstance().getTimeLeft() / 1000)));
            } else {
                chr.getClient().write(CField.getClock((int) (chr.getEventInstance().getTimeLeft() / 1000)));
            }
        }
        if (smr.clock) {
            final Calendar cal = Calendar.getInstance();
            chr.getClient().write((CField.getClockTime(cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE), cal.get(Calendar.SECOND))));
        }
        ...
        if (getSquadBegin() != null && getSquadBegin().getTimeLeft() > 0 && getSquadBegin().getStatus() == 1) {
            chr.getClient().write(CField.getClock((int) (getSquadBegin().getTimeLeft() / 1000)));
        }
        ...
        if (getAllMapObjectSize(MapleMapObjectType.MONSTER) > 0 && (smr.mapid == 280030001 || smr.mapid == 240060201 || smr.mapid == 280030000 || smr.mapid == 240060200 || smr.mapid == 220080001 || smr.mapid == 541020800 || smr.mapid == 541010100)) {
            ...
            chr.getClient().write(CField.musicChange(music));
            //maybe timer too for zak/ht
        }
        if (smr.mapid == 914000000 || smr.mapid == 927000000) {
            chr.getClient().write(CWvsContext.temporaryStats_Aran());
        } else if (smr.mapid == 105100300 && chr.getLevel() >= 91) {
            chr.getClient().write(CWvsContext.temporaryStats_Balrog(chr));
        } else if (smr.mapid == 140090000 || smr.mapid == 105100301 || smr.mapid == 105100401 || smr.mapid == 105100100) {
            chr.getClient().write(CWvsContext.temporaryStats_Reset());
        }
        ...
        if (smr.mapid == 103050900) {
            chr.getClient().write(CField.UIPacket.IntroEnableUI(true));
            chr.getClient().write(CField.UIPacket.UserInGameDirectionEvent(1, 8000));
            chr.getClient().write(CField.UIPacket.UserInGameDirectionEvent(1, 8000));
            chr.getClient().write(CField.UIPacket.UserInGameDirectionEvent(3, 2));
            chr.dropMessage(-1, "On A Rainy Day");
            chr.dropMessage(-1, "The Secret Garden Depths");
            chr.getClient().removeClickedNPC();
            chr.getClient().write(CField.UIPacket.getDirectionStatus(false));
            chr.getClient().write(CField.UIPacket.IntroEnableUI(false));
            chr.getClient().removeClickedNPC();
            chr.dropMessage(-1, "Click on ryden to get your first quest");
        }

        if (permanentWeather > 0) {
            chr.getClient().write(CField.startMapEffect("", permanentWeather, false)); //snow, no msg
        }
        if (smr.footholds.getMovingPlatforms().size() > 0) {
            chr.getClient().write(CField.getMovingPlatforms(this));
        }
        if (environment.size() > 0) {
            chr.getClient().write(CField.getUpdateEnvironment(this));
        }
        //if (partyBonusRate > 0) {
        ...
        if (!smr.needSkillForFly) {
            chr.cancelEffectFromTemporaryStat(CharacterTemporaryStat.Flying);
        }
        if (burningFieldLevel > 0) {
            chr.getClient().write(CField.playSound("Sound/FarmSE.img/boxResult", 100));
            chr.getClient().write(CField.EffectPacket.showBurningFieldTextEffect(getBurningFieldMessage()));
        }
    ...
    public final void broadcastMessage(final User source, final Packet packet, final boolean repeatToSource) {
    ...
    public final void broadcastMessage(final User source, final Packet packet, final Point rangedFrom) {
        broadcastMessage(source, packet, GameConstants.maxViewRangeSq(), rangedFrom);
    }

    public void broadcastMessage(final User source, final Packet packet, final double rangeSq, final Point rangedFrom) {
        charactersLock.readLock().lock();
        try {
                ...
                if (chr != source) {
                    if (rangeSq < Double.POSITIVE_INFINITY) {
                        if (rangedFrom.distanceSq(chr.getTruePosition()) <= rangeSq) {
                            ...
                            chr.getClient().write(packet.Clone());
                        }
                    } else {
                        if (ServerConstants.DEVELOPER_PACKET_DEBUG_MODE) {
    ...
    public void broadcastGMMessage(User source, Packet packet, boolean repeatToSource) {
        broadcastGMMessage(repeatToSource ? null : source, packet);
    }

    private void broadcastGMMessage(User source, Packet packet) {
        charactersLock.readLock().lock();
        try {
            if (source == null) {
                for (User chr : characters) {
    ...
    public void broadcastNONGMMessage(User source, Packet packet, boolean repeatToSource) {
        broadcastNONGMMessage(repeatToSource ? null : source, packet);
    }

    private void broadcastNONGMMessage(User source, Packet packet) {
        charactersLock.readLock().lock();
        try {
            if (source == null) {
                for (User chr : characters) {
     ...
     */
    public void broadcastWhisper(User source, Packet packet, boolean repeatToSource, String msgDestination) {
        broadcastWhisper(repeatToSource ? null : source, packet, msgDestination);
    }

    private void broadcastWhisper(User source, Packet packet, String msgDestination) {
        charactersLock.readLock().lock();
        try {
            if (source == null) {
                for (User chr : characters) {
    ...
    public synchronized void updateBurningField(final long currentTime) {
        ...
        if (burningFieldLevel != currentBurningFieldLevel) {
            ...
            this.broadcastMessage(CField.playSound("Sound/FarmSE.img/boxResult", 100));
            this.broadcastMessage(CField.EffectPacket.showBurningFieldTextEffect(getBurningFieldMessage()));
        }
        // DEBUG ONLY
    ...
    public void setRune(MapleRuneStone m) {
        ...
        spawnAndAddRangedMapObject(m, (MapleClient c1) -> {
            c1.write(CField.RunePacket.spawnRune(rune));
        });
    }
    ...
    public List<Integer> getAllUniqueMonsters() {
        ...
        try {
            mapobjects.get(MapleMapObjectType.MONSTER).values().stream().map((mmo) -> ((MapleMonster) mmo).getId()).filter((theId) -> (!ret.contains(theId))).forEach((theId) -> {
                ret.add(theId);
            });
        } finally {
    ...
    public MapleMonster getMonsterById(int id) {
        mapobjectlocks.get(MapleMapObjectType.MONSTER).readLock().lock();
        try {
            Optional<MapleMapObject> foundNPC = mapobjects.get(MapleMapObjectType.MONSTER).values().stream().filter(obj -> ((MapleMonster) obj).getId() == id).findFirst();
            if (foundNPC.isPresent()) {
                return (MapleMonster) foundNPC.get();
            }
        } finally {
            mapobjectlocks.get(MapleMapObjectType.MONSTER).readLock().unlock();
        }
    ...
    public int countMonsterById(int id) {
        mapobjectlocks.get(MapleMapObjectType.MONSTER).readLock().lock();
        try {
            return (int) mapobjects.get(MapleMapObjectType.MONSTER).values().stream().filter(obj -> ((MapleMonster) obj).getId() == id).count();
        } finally {
            mapobjectlocks.get(MapleMapObjectType.MONSTER).readLock().unlock();
        }
    }
...
// Herbs and Viens
    ...
    public final SpawnPoint addMonsterSpawn(final MapleMonster monster, final int mobTime, final String msg) {
    ...
    public final void changeEnvironment(final String ms, final int type) {
        broadcastMessage(CField.environmentChange(ms, type, 0));
    }
