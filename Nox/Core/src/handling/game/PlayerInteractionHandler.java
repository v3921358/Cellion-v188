        ...
        switch (action) { // Mode
            case CREATE: {
                ...
                if (createType == 4) { // trade//was 3
                    ...
                    final String desc = iPacket.DecodeString();
                    String pass = "";
                    if (iPacket.DecodeByte() > 0) {
                        pass = iPacket.DecodeString();
                    }
                    if (createType == 1 || createType == 2) {
                        final int piece = iPacket.DecodeByte();
            ...
            case HIRED_MERCHANT_MAINTENANCE: {
                ...
                [96 01] [1F] [13] [05] [06 00 31 31 31 31 31 31] [21 A1 07 00] 00
                ?......111111!¡...*/

                iPacket.Skip(1); // 9?
                byte type = iPacket.DecodeByte(); // 6?, appears to be 5 on v170.. TODO: Figure out what is this
                /*if (type != 6) {
                    c.write(CWvsContext.enableActions());
                    return;
                }*/
...
//            case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
            ...
            case REMOVE_ITEM: {
                iPacket.Skip(1); // ?
                int slot = iPacket.DecodeShort(); //0
                final IMaplePlayerShop shop = chr.getPlayerShop();
            ...
            case MERCHANT_BLACKLIST_ADD: {
                final IMaplePlayerShop merchant = chr.getPlayerShop();
                if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr)) {
                    ((HiredMerchant) merchant).addBlackList(iPacket.DecodeString());
                }
                break;
            }
            case MERCHANT_BLACKLIST_REMOVE: {
                final IMaplePlayerShop merchant = chr.getPlayerShop();
                if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr)) {
                    ((HiredMerchant) merchant).removeBlackList(iPacket.DecodeString());
                }
                break;
            }
            case GIVE_UP: {
...
//                    } else {
//                        game.getMCOwner().getClient().write(PlayerShopPacket.getMiniGameRequestTie());
//                    }
...
//                        return;
//                    }
//                    if (iPacket.decodeByte() != game.getTurn()) {
...
//                        return;
//                    }
//                    final int slot = iPacket.decodeByte();
...
//                        } else {
//                            game.getMCOwner().getClient().write(PlayerShopPacket.getMatchCardSelect(turn, slot, fs, turn));
//                        }
...
//            }
            default: {
                ...
                //System.out.println("Unhandled interaction action by " + chr.getName() + " : " + action + ", " + iPacket.toString());
