...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    private final Map<ClientSocket, NPCConversationManager> CMS = new WeakHashMap<>();
    ...
    public boolean hasScript(final ClientSocket c, final int npc, String script) {
    ...
    public void start(ClientSocket c, int npc, String script) {
    ...
    public void action(ClientSocket c, byte mode, byte type, int selection) {
        if (mode != -1) {
...
//                    System.err.println("sel "+ selection);
    ...
    public final void startQuest(ClientSocket c, int npc, int quest) {
    ...
    public void startQuest(ClientSocket c, byte mode, byte type, int selection) {
        final Lock lock = c.getNPCLock();
        final NPCConversationManager cm = CMS.get(c);
        if (cm == null || cm.getLastChatType() != NPCChatType.NULL) {
            return;
    ...
    public void endQuest(ClientSocket c, int npc, int quest, boolean customEnd) {
    ...
    public void endQuest(ClientSocket c, byte mode, byte type, int selection) {
        final Lock lock = c.getNPCLock();
        final NPCConversationManager cm = CMS.get(c);
        if (cm == null || cm.getLastChatType() != NPCChatType.NULL) {
            return;
    ...
    public void startItemScript(ClientSocket c, int npc, String script) {
        final Lock lock = c.getNPCLock();
        lock.lock();
        try {
            if (!CMS.containsKey(c) && c.canClickNPC()) {
    ...
    public void dispose(ClientSocket c) {
    ...
    public NPCConversationManager getCM(ClientSocket c) {
        return CMS.get(c);
    }
}
