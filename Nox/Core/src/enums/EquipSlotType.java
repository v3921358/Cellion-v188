        ...
        if (!isCashItem) {
            ...
            switch (this) {
                case Shield_OrDualBlade: {
                    Item weapon = chr.getInventory(InventoryType.EQUIPPED).getItem((short) Weapon.getSlot());
                    ...
                    if (GameConstants.isKatara(itemid)) {
                        ...
                        MapleInventoryManipulator.unequip(chr.getClient(), (short) Weapon.getSlot(), chr.getInventory(InventoryType.EQUIP).getNextFreeSlot());
                    }
                    break;
                }
                case Weapon_TakingBothSlot_Shield: {
                    Item shield = chr.getInventory(InventoryType.EQUIPPED).getItem((short) Shield_OrDualBlade.getSlot());
                    if (shield != null && GameConstants.isTwoHanded(itemid) && !constants.InventoryConstants.isSpecialShield(shield.getItemId())) {
                        short nextFreeSlot = chr.getInventory(InventoryType.EQUIP).getNextFreeSlot();
                        if (nextFreeSlot == -1) {
                            return -1;
                        }
                ...
                case Pants: {
                    ...
                    final Item top = chr.getInventory(InventoryType.EQUIPPED).getItem((short) Longcoat.getSlot());
                ...
                case Longcoat: {
                    final Item bottom = chr.getInventory(InventoryType.EQUIPPED).getItem((short) Pants.getSlot());
                    if (bottom != null && constants.InventoryConstants.isOverall(itemid)) {
                        if (chr.getInventory(InventoryType.EQUIP).isFull()) {
                            return -1;
                        }
                        MapleInventoryManipulator.unequip(chr.getClient(), (short) Pants.getSlot(), chr.getInventory(InventoryType.EQUIP).getNextFreeSlot());
                    }
                    break;
                }
                ...
                case Ring:
                    for (RingSet s : RingSet.values()) {
                        if (s.id.contains(itemid)) {
                            List<Integer> theList = chr.getInventory(InventoryType.EQUIPPED).listIds();
