...
function start() {
    if (cm.getPlayer().getLevel() < 110) {
        cm.sendOk("There is a level requirement of 110 to attempt Horntail.");
        cm.dispose();
        return;
    }
    ...
    if (prop == null || prop.equals("0")) {
        ...
        if (squadAvailability == -1) {
            ...
            if (time + (6 * 3600000) >= cm.getCurrentTime() && !cm.getPlayer().isGM()) {
                cm.sendOk("You have already went to Horntail in the past 6 hours. Time left: " + cm.getReadableMillis(cm.getCurrentTime(), time + (6 * 3600000)));
                cm.dispose();
                return;
            }
            ...
            if (mode == 1) {
                if (selection == 0) {
                    ...
                    if (cm.getSquad("chaosht") != null) {
                        ...
                        dd.startInstance(cm.getSquad("chaosht"), cm.getMap(), 160113);
                    } else {
                        cm.sendOk("Due to an unknown error, the request for squad has been denied.");
                    }
                    cm.dispose();
