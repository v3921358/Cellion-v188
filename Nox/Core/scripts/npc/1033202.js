...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		cm.sendNextNoESC("Okay. I'll seal the village with my remaining strength...");
		// cm.forceStartQuest(24007, "1");
    } else if (status == 19) {
        cm.sendPlayerToNpc("The elders have fallen into a deep sleep. There's nothing left for you but to join them until the curse wears off...");
    } else if (status == 20) {
        cm.warp(101050000, 18);
        cm.gainExp(4491);
        cm.forceCompleteQuest(24000);
        cm.forceCompleteQuest(24001);
        cm.forceCompleteQuest(24002);
        cm.forceCompleteQuest(24003);
        cm.forceCompleteQuest(24004);
        cm.forceCompleteQuest(24005);
        cm.forceCompleteQuest(24006);
        cm.forceCompleteQuest(24007);
        cm.forceCompleteQuest(24008);
        cm.sendPlayerToNpc("You're awake, and snow no longer falls in Elluel! Is the curse broken? Go outside and find out what's happening.");
	    cm.dispose();
	}
}