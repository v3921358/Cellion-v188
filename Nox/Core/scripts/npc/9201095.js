	...
	if (mode != 1) {
		...
		if (cm.getPlayer().getMapId() == tutorialMapPart1) {
			...
			if (status == 0) {
				...
				cm.sendNextS("Uhh... I'm #h #,\r\nI-I just woke up... I don't know how I got here, where are we?", 3);
			} else if (status == 2) {
				cm.sendNextS("...Ohhh, you must be the new recruit for #dProject: REXION#k. I didn't realize the shuttle came back already.", 1);
			} else if (status == 3) {
				cm.sendNextS("What's #dProject: REXION#k? ... Why don't I remember this?", 3);
			} else if (status == 4) {
				cm.sendNextS("A secret organization from planet #rCerberus#k started #dProject: REXION#k with the intention of rebuilding the #rMaple World#k.", 1);
			} else if (status == 5) {
				cm.sendNextS("#dProject: REXION#k brings in new recruits from the #rMaple World#k and sends them back stronger, with the goal of making the #rMaple World#k a better place. Your memory was wiped as a precaution.", 1);
			} else if (status == 6) {
				cm.sendNextS("WAIT! WE'RE IN SPACE?!", 3);
			} else if (status == 7) {
				cm.sendNextS("Haha, yes we are. Relax, there's nothing to worry about. You'll be back in the #rMaple World#k shortly.", 1);
			} else if (status == 8) {
				cm.sendNextS("First things first, let's make sure you know what you're doing. Come with me.", 1);
			} else if (status == 9) {
			...
			if (!cm.haveItem(tutorialItemCheck)) {
				if (status == 0) {
					cm.sendNextS("Alright, try and kill all these monsters. Let's see what you got!", 1);
				} else if (status == 1) {
				...
				if (cm.getMonsterCount(tutorialMapPart2) > 0) {
					...
					if (status == 0) {
						...
						cm.sendNextS("(I can't believe I just did that.)", 3);
					} else if (status == 2) {
						cm.sendNextS("I'll be waiting for you on the #rSpaceship#k, meet me there when you're ready.", 1);
					} else if (status == 3) {
		...
		// NPC is not used in Part 3 and 4 of tutorial.
			...
			if (!cm.haveItem(4000011, 4)) {
				if (status == 0) {
					...
					cm.sendNextS("You crashed the #rSpaceship#k.....", 2);
				} else if (status == 2) {
				...
				if (status == 0) {
					...
					cm.sendNextS("Yeah, but what about you?", 3);
				} else if (status == 2) {
					...
					cm.sendNextNoESC("#bChannel 8 to Channel 15#k are known as #rBloodless Channels#k. Killing monsters in these channels grants #dAdditional Experience#k as well as larger amounts of #dMaple Points (NX)#k. Although, monsters there #btake reduced damage#k and #rdeal increased damage#k. On top of all that, upon reaching the required experience for the next level, there's a very small chance to #blevel up twice#k or #rnot level entirely#k.");
				} else if (status == 6) {
					if(!(cm.getJobId() == 3001)) { // Not a Demon character.
						...
						cm.sendNextS("As a #bDemon#k, you have the choice of going down two different paths. You can either become a #rDemon Slayer#k or #dDemon Avenger#k.", 3);
					}
...
function completeTutorial() {
	...
	switch(cm.getJobId()) {
		...
		case 501: // Cannoneer
			...
			break;
		case 508: // Jett
			cm.gainItem(1352300, 1);  // Core Frag (Jett Secondary Weapon)
			cm.gainItem(1352820, 1);  // Basic Fist (Jett Secondary Weapon)
			//cm.warp(120000000, 0); // Nautilus Harbor
			cm.warp(101071300, 0); // Home Map
			break;
		case 1000: // Cygnus
