function enter(pi) {
	...
	if (map == 807300110) {
			if (pi.getMap().getAllMonstersThreadsafe().size() == 1) {
			    ...
			    pi.playerMessage("You sir are not yet ready to take on the Oda Clan Ranmaru...");
			//	pi.worldMessage(6, "[Azwan] " + pi.getPlayer().getName() + " finished the Azwan Liberation of Hilla's Gang in Channel "+ pi.getClient().getChannel() +".");
			 ...
			 pi.gainExp(45000000);
		pi.playerMessage(6, "Congratz on defeating [Chaos] Ranmaru.");
								if (pi.isLeader());
		pi.worldMessage(6, "[Chaos Mode] Ranmaru has been defeated");
		pi.warp(211041700,0);
	    }
