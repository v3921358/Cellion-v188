...
function setup(eim, leaderid) {
	...
	if (em.getProperty("res" + i).equals("0")) {
	    var available = em.getMapFactory().getMap(926013100 + i);
...
function playerEntry(eim, player) {
	...
	var available = em.getMapFactory().getMap(926013100 + eim.getPropertyEx("Selected"));
	if (available == null) {
	    eim.unregisterPlayer(player);
...
function scheduledTimeout(eim) {
    ...
    var map = 926013100 + ((stage + 1) * 100);

    var selid = map + eim.getPropertyEx("Selected");
    var available = em.getMapFactory().getMap(selid);
