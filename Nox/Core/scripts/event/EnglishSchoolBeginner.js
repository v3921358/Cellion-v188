...
function setup() {
    ...
    var eim = em.newInstance("english_b");
    
    eim.setProperty("allow_warpback", "false");
    eim.setProperty("Pass", "false");
    eim.setPropertyEx("GivePass", 2);
    
    eim.startEventTimer(300000);

    var map = eim.getMapFactory().getMap(702090101);
    map.resetMap();
...
function beginQuest(eim) { // Custom function
    eim.broadcastMapEffect(eim.getMapFactory().getMap(702090101), 0)
}
